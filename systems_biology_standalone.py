...
def build_rag_context(query, docs, limit=3000):
    ...
    _, idx = st.session_state.vector_index.search(query_emb, k=min(3, len(docs)))
...
def analyze_kegg_pathway(org_code, target_pathway, max_other_paths, full_df=None):
    ...
    genes_in_target = full_df[full_df["Pathway ID"] == target_pathway]["Gene ID"].unique()
    gene_to_pathways = full_df.groupby("Gene ID")["Pathway ID"].apply(set).to_dict()
...
def calculate_hub_genes(df, min_pathways=3):
    ...
    hub_genes = gene_pathway_count[gene_pathway_count >= min_pathways].sort_values(ascending=False)
    return hub_genes
...
def build_graph_from_kegg(selected_pathways, full_df, pleiotropy_threshold):
    ...
    # Find shared genes
    shared_genes = [gene for gene, pathways in gene_to_pathways.items() if len(pathways) >= 2]
...
selected_org_display = st.sidebar.selectbox(
    "Select Organism",
    options=org_options,
    index=org_options.index("hsa - Homo sapiens (human)") if "hsa - Homo sapiens (human)" in org_options else 0
)
...
if analysis_mode == "Pathway-Centric":
    ...
    pathway_options = [f"{pid} - {pname}" for pid, pname in sorted(pathway_dict.items())]
    ...
    if pathway_mode == "Single Pathway":
        ...
        if selected_pathway_display:
            selected_pathway_id = selected_pathway_display.split(" - ")[0].strip()
        ...
        selected_pathways = [p.split(" - ")[0].strip() for p in selected_pathway_displays]
...
if analysis_mode == "Pathway-Centric":
    ...
    with tab1:
                ...
                if pathway_mode == "Single Pathway":
                    ...
                    for pathway_id in selected_pathways:
                        try:
                            ...
                            st.error(f"Error analyzing pathway {pathway_id}: {str(e)}")
                            continue
                    ...
                    pathway_gene_sets = {pid: set(all_results[pid]['genes']) for pid in all_results.keys()}
                    
                    if len(pathway_gene_sets) > 1:
                        shared_genes = set.intersection(*pathway_gene_sets.values())
                    else:
                        shared_genes = list(pathway_gene_sets.values())[0] if pathway_gene_sets else set()
        ...
        if st.session_state.analysis_results:
            if st.session_state.analysis_results.get('multi_pathway'):
                ...
                st.metric("Shared Genes Across All Pathways", len(shared_genes))
                ...
                for pathway_id in selected_pathways:
                    if pathway_id in all_results:
                        pathway_name = pathway_dict_display.get(pathway_id, pathway_id)
                        ...
                        with st.expander(f"📊 {pathway_name} ({len(genes)} genes)"):
                            for n in range(max_other_pathways + 1):
                                ...
                                if genes_list:
                                    st.write(f"**Genes with {n} other pathways:** {len(genes_list)}")
                                    df_display = pd.DataFrame([
                                        {
                                            "Gene ID": g['Gene ID'],
                                            "Other Pathways": ", ".join(g['Other Pathways'][:2]) +
                                            (f" ... (+{len(g['Other Pathways'])-2})" if len(g['Other Pathways']) > 2 else "")
                                        }
                ...
                st.success(f"✅ Found {len(genes_in_target)} genes in pathway {selected_pathway_id}")
                            ...
                            df_display = pd.DataFrame([
                                {
                                    ...
                                    (f" ... (+{len(g['Other Pathways'])-3} more)" if len(g['Other Pathways']) > 3 else "")
                                }
                                for g in genes
                            ])
                            st.dataframe(df_display, width='stretch')
    ...
    with tab2:
        ...
        graph_data = build_graph_from_kegg(selected_pathways, full_df, pleiotropy_threshold)
        ...
        gene_pathway_counts = full_df.groupby("Gene ID")["Pathway ID"].nunique().to_dict()
        ...
        for node in nodes:
            ...
            G.add_node(node_id, label=node_id, color=color, size=int(size), title=title)
        ...
        if pathway_mode == "Multiple Pathways (Comparison)":
            net.repulsion(node_distance=250, central_gravity=0.15, spring_length=250, spring_strength=0.04, damping=0.09)
        else:
        ...
        if pathway_mode == "Multiple Pathways (Comparison)":
            ...
            st.info(f"💡 **Node Size**: {'Proportional to pathway count' if use_proportional_sizing else 'Fixed size'}")
        else:
            cols = st.columns(3)
            with cols[0]:
                st.markdown("🟠 **Pathway**")
    ...
    with tab3:
        ...
        with col1:
            ...
            if not hub_genes.empty:
                ...
                st.info(f"No hub genes found with >= {hub_gene_threshold} pathways")
        
        with col2:
            st.subheader("Pleiotropic Genes")
            gene_pathway_counts = full_df.groupby("Gene ID")["Pathway ID"].nunique()
            pleiotropic_genes = gene_pathway_counts[gene_pathway_counts >= pleiotropy_threshold].sort_values(ascending=False)
            ...
            if not pleiotropic_genes.empty:
                ...
                st.info(f"No pleiotropic genes found with >= {pleiotropy_threshold} pathways")
    ...
    with tab4:
        ...
        if st.session_state.analysis_results:
            genes_in_target = st.session_state.analysis_results.get('genes', [])
            
            if not genes_in_target:
                st.warning("No gene data available. Please run pathway analysis first.")
            else:
                ...
                if st.button("🔬 Simulate Knockout"):
                    impact_score, pathway_details = simulate_knockout_impact(selected_gene, full_df)
                    
                    st.metric("Impact Score (Affected Pathways)", impact_score)
                    
                    st.subheader(f"Pathways Affected by {selected_gene} Knockout")
    ...
    with tab5:
        ...
        if st.session_state.analysis_results:
            ...
            if not genes_in_target:
                st.warning("No genes found. Please run pathway analysis first.")
            else:
                st.subheader(f"🧬 Pathway Biological Context: {selected_pathway_name}")
                
                if st.button("Fetch Pathway Context"):
                    with st.spinner("Fetching PubMed literature..."):
                        pathway_docs = fetch_pubmed(f"{selected_pathway_name} pathway")
                        ...
                        if pathway_docs:
                            ...
                            st.text_area("Pathway Context", context, height=300)
                        else:
                            st.warning("No literature found for this pathway")
                
                st.markdown("---")
                
                st.subheader(f"🔬 Gene-Level Analysis (Limited to {gene_rag_limit} genes)")
                st.info(f"Gene-level analysis is limited to {gene_rag_limit} genes for performance.")
                        ...
                        with col1:
                            if st.button(f"Fetch Biological Context", key=f"bio_{gene}"):
                                with st.spinner(f"Fetching context for {gene}..."):
                                    gene_docs = fetch_pubmed(f"{gene} {selected_org_name}")
                                    ...
                                    if gene_docs:
                                        ...
                                        st.text_area("Biological Context", context, height=200, key=f"bio_text_{gene}")
                                    else:
                        ...
                        with col2:
                            if st.button(f"Fetch Knockout Phenotype", key=f"ko_{gene}"):
                                with st.spinner(f"Fetching knockout data for {gene}..."):
                                    knockout_docs = fetch_pubmed(f"{gene} knockout {selected_org_name}")
                                    ...
                                    if knockout_docs:
                                        ...
                                        st.text_area("Knockout Phenotype", context, height=200, key=f"ko_text_{gene}")
                                    else:
...
# =====================================================
    ...
    with tab1:
        ...
        if pathway_count == 1:
            st.info(f"🟢 **Pathway-Specific Gene**: {selected_gene_id} is specific to one pathway")
        elif pathway_count < pleiotropy_threshold:
            st.warning(f"🟡 **Multi-Pathway Gene**: {selected_gene_id} participates in {pathway_count} pathways")
        else:
            st.error(f"🔴 **Pleiotropic Hub Gene**: {selected_gene_id} is involved in {pathway_count} pathways (≥ threshold)")
    ...
    # TAB 2: GENE NETWORK
    with tab2:
        st.header(f"🕸️ Gene Network: {selected_gene_id}")
        
        # Get pathways for this gene
        gene_pathways = full_df[full_df["Gene ID"] == selected_gene_id]["Pathway ID"].unique()
        
        # Build graph
        graph_data = build_graph_from_kegg(list(gene_pathways), full_df, pleiotropy_threshold)
        nodes = graph_data["nodes"]
        edges = graph_data["edges"]
        gene_to_pathways = graph_data.get("gene_to_pathways", {})
        ...
        net = Network(height="700px", width="100%", bgcolor="#ffffff", font_color="black")
        net.from_nx(G)
        net.repulsion(node_distance=200, central_gravity=0.2, spring_length=220, spring_strength=0.05, damping=0.09)
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Pathways", len([n for n in nodes if n["label"] == "Pathway"]))
        with col2:
            st.metric("Co-pathway Genes", len([n for n in nodes if n["label"] == "Gene"]) - 1)
        ...
        with col1:
            ...
            if st.button("Fetch Biological Context"):
                with st.spinner(f"Fetching context for {selected_gene_id}..."):
                    gene_docs = fetch_pubmed(f"{selected_gene_id} {selected_org_name}")
        ...
        with col2:
            ...
            if st.button("Fetch Knockout Data"):
                with st.spinner(f"Fetching knockout data for {selected_gene_id}..."):
                    knockout_docs = fetch_pubmed(f"{selected_gene_id} knockout {selected_org_name}")
